...
#' 
#' @param inputTS A time series with only data.
#' @param method A string showing the method used to calculate mean value, e.g., "meanAnnualPreci".
#' @param yearIndex A list showing the year index of the time series.
#' @param monthIndex A list showing the month index of the time series.
#' @param fullResults A boolean showing whether the full results are shown.
...
getMeanPreci <- function(inputTS, method = NULL, yearIndex = NULL, monthIndex = NULL,
  ...
  if(method == 'meanAnnualPreci'){
    ...
    if(fullResults == TRUE) output <- annualPreci else output <- mean(annualPreci, na.rm=)
