...
Xu, Yuanchao(2015). hyfo: Hydrology and Forecasting R Package for Data Analysis and Visualization. Retrieved from http://yuanchao-xu.github.io/hyfo/
...
You can also use `loadGridData{ecomsUDG.Raccess}` or `loadECOMS{ecomsUDG.Raccess}` to load NetCDF file, their input and output can cope with hyfo.  Pacakage[`{ecomsUDG.Raccess}`](https://github.com/SantanderMetGroup/ecomsUDG.Raccess)is designed for getting access to different dataset, and also can load grid file (like netcdf file) directly.

* Most data visualization tool provides the output that can be directly re-plot by `ggplot2`, if `output = 'ggplot'` is assigned in the argument of the function, which will be easier for the users to generated series/multiple plots afterwards, hyfo also provides different multiplot tools end with `_comb`, details can be found in the user mannual.
...
```{r, fig.show='hold', fig.height=4, fig.width=7}
a <- getPreciBar(TS = TS, method = 'spring', fullResults = FALSE)
a
```
Get the series of spring precipitation, keep default.
```{r, fig.show='hold', fig.height=4, fig.width=7}
